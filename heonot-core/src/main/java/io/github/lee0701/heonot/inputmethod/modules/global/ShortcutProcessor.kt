...
open class ShortcutProcessor : InputMethodModule() {
        ...
        HeonOt.INSTANCE?.treeEvaluator?.let { evaluator ->
            for (shortcut in shortcuts) {
                ...
                if (keyStroke.keyCode == keyCode
                    ...
                    evaluator.variables = HeonOt.INSTANCE?.variables
                    val result = evaluator.eval(shortcut.treeNode)
                    when (shortcut.mode) {
                        Shortcut.MODE_CHANGE -> HeonOt.INSTANCE?.changeInputMethod(result.toInt())
